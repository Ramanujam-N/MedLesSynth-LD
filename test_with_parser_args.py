...
def save_image(i,image,label,gndtruth,affine,dice,combined_check,date):
    ...
    nib.save(image1, './images/'+date+'/'+combined_check+'/'+'Testing'+str(i)+'_'+str(dice)+'_FLAIR.nii.gz')
...
def test(system_data_path,mode,date,model,data,criterion,workers=4,batch=8,factor=1,device=0,pretrained=None,scale_factor =1,hyper_parameters=None,exp_id='default',sim_path=None,optthresh=False,size=(128,128,128),no_crop=False,combined_check="brats"):
    ...
    with tqdm(range(len(testloader))) as pbar:
        for i, data in zip(pbar, testloader):
            with torch.no_grad():
                ...
                # save_image(i,image,pred,label,data['affine'][0],np.round(dice, 2),combined_check,date)
...
if(__name__ =="__main__"):
    ...
    parser.add_argument("-mode",default='S',choices=['S','SS','SSDA','DA','FT','PT','FPI'],
