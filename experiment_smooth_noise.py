...
def save_image(i,image,label,label_wothresh,gndtruth,affine,dice,data,mode,exp_id,date):
    os.makedirs(f'./images/smoothnoise/{date}/{mode}/{data}/{exp_id}/',exist_ok=True)
    if(affine==None):
        plt.imsave(f'./images/smoothnoise/{date}/{mode}/{data}/{exp_id}/Testing{str(i)}_{str(dice)}_image.png',image,cmap='gray')
        plt.imsave(f'./images/smoothnoise/{date}/{mode}/{data}/{exp_id}/Testing{str(i)}_{str(dice)}_outmask.png',label,cmap='gray')
        plt.imsave(f'./images/smoothnoise/{date}/{mode}/{data}/{exp_id}/Testing{str(i)}_{str(dice)}_outmask_wothresh.png',label_wothresh,cmap='gray')
        plt.imsave(f'./images/smoothnoise/{date}/{mode}/{data}/{exp_id}/Testing{str(i)}_{str(dice)}_manualmask.png',gndtruth,cmap='gray')

    else:
        image1 = nib.Nifti1Image(image, affine=affine)
        nib.save(image1, f'./images/smoothnoise/{date}/{mode}/{data}/{exp_id}/Testing{str(i)}_{str(dice)}_FLAIR.nii.gz')

        output1 = nib.Nifti1Image(label.astype(np.single), affine=affine)
        nib.save(output1, f'./images/smoothnoise/{date}/{mode}/{data}/{exp_id}/Testing{str(i)}_{str(dice)}_outmask.nii.gz')

        output2 = nib.Nifti1Image(label_wothresh.astype(np.single), affine=affine)
        nib.save(output2, f'./images/smoothnoise/{date}/{mode}/{data}/{exp_id}/Testing{str(i)}_{str(dice)}_outmask_wothresh.nii.gz')

        output3 = nib.Nifti1Image(gndtruth.astype(np.single), affine=affine)
        nib.save(output3, f'./images/smoothnoise/{date}/{mode}/{data}/{exp_id}/Testing{str(i)}_{str(dice)}_manualmask.nii.gz')


def test(system_data_path,mode,date,model,data,criterion,workers=4,batch=8,factor=1,device=0,pretrained=None,scale_factor =1,hyper_parameters=None,exp_id='default',sim_path=None,optthresh=False,size=(128,128,128),no_crop=False,combined_check="brats",not_best=False,thresh=0.5,sigma_noise=1,sigma_smooth=1,setting='smooth'):
    ...
    which_data = data
    model_name = model
    _,_,datadict_test = helper_smooth_noise(system_data_path,which_data=data,size=size,no_crop=no_crop,sigma_smooth=sigma_smooth,sigma_noise=sigma_noise,setting=setting)
    ...
    os.makedirs(f'./csv_files/{date}/{mode}/{data}/{setting}',exist_ok =True)
        ...
        for i, data in zip(pbar, testloader):
            with torch.no_grad():
                ...
                if('affine' not in data.keys()):
                    save_image(i,image,pred,pred_wothresh,label,None,np.round(dice, 2),which_data,mode,exp_id+'_'+setting+'_'+str(sigma_noise)+'_'+str(sigma_smooth),date)
                else:
                    save_image(i,image,pred,pred_wothresh,label,data['affine'][0],np.round(dice, 2),which_data,mode,exp_id+'_'+setting+'_'+str(sigma_noise)+'_'+str(sigma_smooth),date)
    ...
    pd.DataFrame({'Dice':dice_list,'Hausdorff':hau_list,'TPR':tpr_list,'TNR':tnr_list,}).to_csv(f'./csv_files/{date}/{mode}/{which_data}/{setting}/'+model_name+'_'+which_data+'_'+mode+'_'+exp_id+'_'+date+'_'+setting+'_'+str(sigma_noise)+'_'+str(sigma_smooth)+'_'+str(pretrained)+'.csv')
...
if(__name__ =="__main__"):
    ...
    parser.add_argument("-sigma_smooth",default=0.1,type=float,)
    parser.add_argument("-sigma_noise",default=1,type=float,)
    parser.add_argument("-setting",default='smooth',choices=['smooth','noise'])
    ...
    data_addresses = {63:'/mnt/d1bdf387-8fd2-4f57-8c8a-eba9ef0baff6',64:'/mnt/70b9cd2d-ce8a-4b10-bb6d-96ae6a51130a',112:'/mnt/04d05e02-a59c-4a91-8c16-28a8c9f1c14f',131:'/mnt/a4ef64ea-1b6b-4423-b1d2-4794d2e97289'}
