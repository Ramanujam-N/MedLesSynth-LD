...
class LesionGeneration3D(Dataset):
    ...
    def normalize(self,image,range_min=0,range_max=1):
        image_min = image.min()
        image_max = image.max()
        range_max = np.random.uniform(range_max-0.5*range_max,range_max)
    ...
    def gaussian_small_shapes(self,image_mask,small_sigma = [1,1,1]):
        ...
        index = -1
        while(index==-1):
            noise = np.random.normal(size = image_mask.shape)
    ...
    def shape_generation(self,scale_centroids,centroid_main,num_ellipses,semi_axes_range,perturb_sigma=[1,1,1],image_mask=1):
        ...
        random_major_axes = np.random.uniform(semi_axes_range[0],semi_axes_range[1],(num_ellipses,1))
        random_minor_axes = np.concatenate([np.ones((num_ellipses,1)),np.random.uniform(0.3,1,size = (num_ellipses,2))],1) #0.1 to 0.8
        ...
        random_rot_angles = np.random.uniform(-np.pi/2,np.pi/2,size = (num_ellipses,3))
        ...
        noise_b_box = np.random.normal(size = image_mask.shape)
    ...
    def simulation(self,image,inter_image,image_mask,num_lesions=3,gt_mask=None,roi_mask=None):
        ...
        if(image.shape !=inter_image.shape):
            inter_image = skiform.resize(image, image.shape, order=1, preserve_range=True ,anti_aliasing=True)
        # roi = skimage.morphology.binary_erosion(image_mask,skimage.morphology.ball(10))*(image>0.1) #15
        roi = ndimage.binary_erosion(image_mask,skimage.morphology.ball(5))*(image>0.1) # 12 #15
        ...
        for i in range(num_lesions):
            ...
            if(self.which_data=='lits'):
                smoothing_mask = np.random.uniform(0.2,0.3)
                
            tex_sigma = np.random.uniform(0.4,0.7)
            range_min = np.random.uniform(0,0.5)
